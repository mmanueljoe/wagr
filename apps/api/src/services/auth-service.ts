...
export async function registerEmployer(input: RegisterEmployerInput): Promise<AuthResult> {
  ...
  // Same UUID as the auth user. If the employer insert fails we delete the
  // auth user — no orphan accounts.
  ...
  const user: AuthUser = { id: data.user.id, employer_id: data.user.id, email }
...
export async function loginEmployer(input: LoginEmployerInput): Promise<AuthResult> {
  ...
  const user: AuthUser = { id: data.user.id, employer_id: employer.id, email: employer.email }
  const sessionId = await createSession({
    user_id: user.id,
    employer_id: user.employer_id,
    email: user.email,
...
export function getMe(session: SessionData): AuthUser {
  return { id: session.user_id, employer_id: session.employer_id, email: session.email }
}
