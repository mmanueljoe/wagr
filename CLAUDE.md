...
## Architecture & Security Standards (Wagr is fintech)

Wagr moves real wages to real workers' MoMo wallets. That puts us in a
different threat class from a typical SaaS dashboard. These rules exist
so I don't default to consumer-SaaS shortcuts. Each one says **what** to
do and **why**.

Source-of-truth: OWASP ASVS for session/auth, Stripe's API design docs for
money operations, RFC documents for protocols. When in doubt I read these,
not blog posts.

### Auth boundary

- **The browser never talks to Supabase Auth or Moolre directly.** All
  third-party calls flow through the Wagr api. Browser → Wagr api → provider.
  This is the BFF (Backend-for-Frontend) pattern. See ADR 007.
- **Real tokens (Supabase JWTs, refresh tokens, Moolre keys) never reach the
  browser.** Sessions live in Redis on the api, keyed by an opaque session
  ID. The browser only sees the opaque ID, in an HttpOnly cookie.
- **Session cookies are `HttpOnly`, `SameSite=Lax`, `Secure` in prod.** No
  exceptions. JavaScript on the page literally cannot read them — that's
  the point.
- **`@supabase/ssr` on the web is not used.** The web speaks to the Wagr
  api only. Supabase Auth admin SDK lives on the api.

### Money operations

- **All amounts stored and computed as integer minor-units (pesewas).**
  GHS 11.00 = `1100`. Never `number`-typed floats, never `11.0`. Convert to
  display strings at the UI boundary only. Postgres stays `numeric(12,2)`
  for human readability; the marshalling layer multiplies/divides by 100.
  See ADR 008.
- **Every money-moving request carries an idempotency key.** Generated by
  the api, stored before the Moolre call, replay-safe. Re-running must
  never double-disburse. See ADR 009.
- **A worker can have at most one `pending` advance at a time.** Enforced
  by a unique partial Postgres index. Race conditions die at the DB layer,
  not in app code.
- **LLM output never drives financial decisions.** GPT-4o writes UX text
  (payslip closing line, credit-flag explanation). Numbers, eligibility,
  flag/no-flag are deterministic code only.

### Validation, authorization, audit

- **Server-side validation is the source of truth.** Client-side Zod is for
  UX feedback only. The api re-validates every body at the route boundary
  before any side effect.
- **Every employer-scoped endpoint explicitly checks
  `resource.employer_id === session.employer_id`.** RLS is defense in depth
  (still enabled on every table), but the api owns authorization.
- **Every state change that affects money or identity → row in `audit_log`.**
  Login, logout, advance request, disbursement, employee add/deactivate,
  PIN change, float top-up, recovery. Structured `metadata` jsonb so we
  can reconstruct any incident. See ADR 010.
- **Worker PINs are bcrypt-hashed with per-attempt rate limiting and
  lockout.** 5 wrong attempts in 10 minutes → 30 min lock. Every attempt
  is audit-logged. PIN values never logged.

### External calls (Moolre, OpenAI, Supabase)

- **Every external call has an explicit timeout.** No unbounded `fetch`.
  Default 10s, override per call. No long-running work in HTTP handlers —
  queue it.
- **Moolre webhooks are HMAC-verified.** Reject unsigned or wrong-signature
  bodies. Replay-protect with the request's own nonce/timestamp.
- **Rate-limit auth, advance-request, and webhook endpoints.** Backed by
  Redis. Defaults: auth 5/min/IP, advance 1/min/employee, webhooks sane
  ceiling per source.

### PII and logging

- **Never log phone, MoMo number, salary, full name, or PIN.** Redact
  before `logger.info`. Never put PII in URL params or query strings.
- **Phone numbers stored E.164** (`+233\d{9}` for Ghana). Normalised at
  the input boundary, never accepted in mixed formats.
- **Timestamps are UTC `timestamptz` in DB, UTC in code.** Convert at
  display only. Pass `now` as a parameter to keep functions pure.

### Secrets

- **`.env` never committed.** `.env.example` shows shape only.
- **Production secrets come from Vercel/Railway dashboards**, not from
  the repo. Documented rotation every 90 days in
  [docs/architecture/secrets.md](docs/architecture/secrets.md) (TODO).

### Defence in depth (not optional)

- **Postgres CHECK constraints + NOT NULL + foreign keys with the right
  ON DELETE behaviour** on every financial table. The DB is the last
  guard if app code is wrong.
- **RLS stays enabled** on `employers`, `employees`, `advance_requests`,
  `repayments`, `audit_log` — even though the api uses the service-role
  key. Belt and braces.
- **Content-Security-Policy header in prod** — `default-src 'self'`. Added
  with the first production deploy.
- **No third-party scripts on `/dashboard/*`.** Analytics belong on the
  landing page only.

### Things this rules out by construction

- Storing the Supabase JWT in the browser.
- "Remember me" via long-lived access tokens in the browser.
- Catching a Moolre failure and pretending success.
- Using `any` to silence a Supabase or Moolre type error.
- Adding a "soft auth" or "guest mode" anywhere on `/dashboard/*`.
- Logging a full Moolre response in production.
